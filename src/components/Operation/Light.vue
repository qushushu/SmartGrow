<template>
    <div>
        ...
        <!-- 种植植物灯开关（不分左右） start -->
        <a-card v-loading="fullscreenLoading" :class="{isloading: fullscreenLoading}" v-if="isPlant">
            <div v-if="plantDigInfo.length" v-for="k in 10">
                <div v-if="plantDigInfo[k - 1]" class="space-btm1">
                    <div class="light-headerbar">
                        <h5 class="light-tit light-tit_spe">第{{k}}组植物灯</h5>
                        <div class="light-mode-switch">
                            <span style="margin-right: 16px;">亮度：{{plantLightArr[k - 1]}}
                                <a v-if="planManuInfo[k - 1][0].value == 0 ? false : true" href="javascript:;" class="el-icon-edit lightEdit" @click="showEditLightLayer(k - 1,plantLightArr[k - 1])"></a>
                            </span>
                            <span>手动模式：<el-switch v-model="planManuInfo[k - 1][0].value == 0 ? false : true" @change="changeLightGroupMode(k-1)"></el-switch></span>
                            <el-button type="primary" size="small" :disabled="(runInfo.dig.AUTO === 1) || (planManuInfo[k - 1][0].value == 0)" style="margin-left: 8px;" @click="showDialog(k)">参数设置</el-button>
                        </div>
                    </div>
                    <el-table :data="tableData" size="small" border>
                        <el-table-column prop="title"></el-table-column>
                        <el-table-column>
                            <template slot-scope="scope">红光开关：
            ...
            <el-empty v-if="!planManuInfo.length"></el-empty>
        </a-card>
        <!-- 种植植物灯开关 end -->
...
<style scoped>
    ...
    .light-headerbar h5 {line-height: 30px;margin-bottom: 0;}
