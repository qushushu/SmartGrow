    ...
    <div class="shuixiang" v-if="showBox">
        <div id="hskedu">
            <div class="kd" 
                ...
                :adjnum="'回水液位调节启动值：' + (maxNum - key) + 'cm'" 
                :tarnum="'回水液位目标值：' + (maxNum - key) + 'cm'" 
                :curnum="'当前回水液位：' + (maxNum - key) + 'cm'"
                :snum = "maxNum - item + 1"
                v-if="showBox"
            ></div>
        </div>
...
#hskedu {position: absolute;left: 0;top: 20px;width: 100%;height: calc(100% - 40px);z-index: 4;}
    ...
    export default {
        ...
        methods: {
            resetData() {
                this.showBox = true;
                setTimeout(()=> {
                    let aH = document.querySelector("#hskedu").offsetHeight;
