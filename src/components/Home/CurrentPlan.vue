        ...
        <div class="mycard-body">
            ...
            <div class="normal-list">
              <a-descriptions class="form-text" size="middle" :column="2">
                <a-descriptions-item :label="$t('message.定做育苗')">{{plantName}} </a-descriptions-item>
                <a-descriptions-item :label="$t('message.EC目标值')">{{EC_TV}} </a-descriptions-item>
                <a-descriptions-item :label="$t('message.定植日期')">{{start_time}} </a-descriptions-item>
                <a-descriptions-item :label="$t('message.PH目标值')"> {{PH_TV}} </a-descriptions-item>
                <a-descriptions-item :label="$t('message.运行方案')">{{scheme_name}}</a-descriptions-item>
                <a-descriptions-item :label="$t('message.温度目标值')">{{temp_day}}（日） / {{temp_night}}（夜）</a-descriptions-item>
                <a-descriptions-item :label="$t('message.生产周期')">{{grow_cycle}} 天</a-descriptions-item>
                <a-descriptions-item :label="$t('message.湿度目标值')">{{humidity_day}}（日） / {{humidity_night}}（夜）</a-descriptions-item>
                <a-descriptions-item :label="$t('message.方案备注')">{{remark}} </a-descriptions-item>
                <a-descriptions-item :label="$t('message.日出/日落')">{{SUNRIZE}} / {{SUNSET}}</a-descriptions-item>
                <a-descriptions-item :label="$t('message.当前阶段')">{{stage_name}}</a-descriptions-item>
                <a-descriptions-item :label="$t('message.CO2目标值')">{{CO2}}</a-descriptions-item>
              </a-descriptions>
            </div>
            <!-- PC端详细信息 end -->
        </div>
