	...
	<div class="ym-main">
		<a-card>
			...
			<a-form-model layout="inline" class="space-btm1">
				<a-form-model-item><a-input-search :placeholder="$t('message.请输入设备型号')" enter-button v-model="model" @search="getList" /></a-form-model-item>
				<a-form-model-item><a-button type="solid"  @click="handleEdit">{{$t('message.新增')}}</a-button></a-form-model-item>
			</a-form-model>
			<!-- 搜索与新增 end -->
			<!-- 表格 start -->
			<el-table ref="multipleTable" :data="tableData" border  size="small">
			    <el-table-column prop="model_code" :label="$t('message.型号编号')"></el-table-column>
			    <el-table-column prop="model" :label="$t('message.设备型号')" width="120"></el-table-column>
			    <el-table-column prop="version" :label="$t('message.版本')" width="80"></el-table-column>
			    <el-table-column prop="description" :label="$t('message.描述')"></el-table-column>
			    <el-table-column prop="btns" :label="$t('message.操作')" width="223">
