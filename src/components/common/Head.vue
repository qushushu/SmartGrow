	...
	<header id="group-header">
		<el-row type="flex" justify="space-between" align="middle" class="head-box">
			<el-col>
				...
				<!-- title start -->
				<router-link to="/" class="logo mobile-hide"><h1>{{$t(logoTitle)}}</h1></router-link>
				<!-- title end -->
			</el-col>
			<el-col>
				<el-row type="flex" justify="end" align="middle">
					...
					<el-dropdown @command="handleCommand1" trigger="click" v-if="userInfo.token">
						...
						<el-dropdown-menu slot="dropdown">
						    ...
						    <el-dropdown-item command="logout">{{$t("message.退出")}}</el-dropdown-item>
						</el-dropdown-menu>
					</el-dropdown>
					<!-- 用户退出 end -->
					<!-- 切换语言 start -->
		...
		<!-- 计划列表弹窗 end -->
		<!-- 用户登录 start -->
		<el-dialog :visible.sync="userLoginDialogVisible" :title="$t('message.退出')" :width="loginDialogWidth" center>
...
<script>
	...
	export default {
		...
		methods: {
			tgMobileNav() {
				document.querySelector(".el-menu").classList.toggle("mobile-show")
			},
			// 登录
